...
\begin{algorithm}
	\caption{AdaBoost}
	\begin{algorithmic}[1]
		\STATE $w \leftarrow (1/n, ..., 1/n)$ \COMMENT{wagi początkowe}
		\FOR{$t = 1$ \TO $T$}
			\STATE $h^{(t)} \leftarrow \text{Base}(\mathcal{D}_n, \boldsymbol{w}^{(t)})$ \COMMENT{klasyfikator bazowy}
			\STATE $\varepsilon^{(t)} \leftarrow \sum\limits_{i = 1}^n w_i^{(t)} \mathbbm{1} \{ h^{t}(\boldsymbol{x}_i) \neq y_i \}$ \COMMENT{błąd ważony klasyfikatora bazowego}
			\STATE $\alpha^{(t)} \leftarrow \frac{1}{2} \ln( \frac{1-\varepsilon^{(t)}}{\varepsilon^{(t)}} )$ \COMMENT{współczynnik klasyfikatora bazowego}
			\FOR{$i = 1$ \TO $n$} 
				\COMMENT{ponowne ważenie punktów treningowych}
			...
			\ENDFOR
		\ENDFOR
		\RETURN $f^{(T)}(\cdot) = \sum\limits_{t = 1}^T \alpha^{(t)}h^{(t)}(\cdot)$
