...
Cecha GIST (\emph{ang. streszczenie}) jest globalną cechą obrazu, która charakteryzuje całą scenę obrazu. Globalne cechy oparte są na konfiguracji skal przestrzennych szacowanych bez użycia operacji grupowania ani segmentyzacji. Ekstrakcja cech GIST jest bardzo szybka. Podejście to nie jest alternatywą dla lokalnych cech obrazu ale może stanowić dodatkową pomoc przy rozpoznawaniu obiektów na nieuporządkowanej scenie.\cite{OLIVA06}

Sensowność tego podejścia jest poparta badaniami ludzkiej percepcji\cite{SCHYNS94}, które wykazały, że człowiek jest w stanie rozpoznać znaczenie nawet bardzo skomplikowanej sceny w krótkim czasie, nawet wówczas gdy obraz ten jest nieostry.

Nie istnieje jeden model GIST. Oliva i Torralba w 2002 roku zaproponowali model, w którym rozkład przestrzenny widma został opisany przy pomocy okienkowej transformaty Fouriera (WFT).

%Deskryptory GIST są obecnie coraz częściej stosowane w rozpoznawaniu sceny. 
