...
Przestrzeń cech jest wielowymiarowa i określenie dla niej hiperpłaszczyzny jest bardzo trudne obliczeniowo. Zamiast tego stosuje się funkcje jądrowe (\emph{ang. kernel functions}). Rozwiązanie klasyfikacji jest wówczas sumą ważoną funkcji jądrowych oszacowanych w miejscu wektorów nośnych.
...
Na jakość klasyfikacji może mieć wpływ zarówno niedouczenie jak i przeuczenie klasyfikatora. Niedouczenie, nazywane też niewystarczającym dopasowaniem (\emph{ang. underfitting}) występuje wówczas gdy posługujemy się zbyt ogólnym, uproszczonym modelem. Zbyt mała ilość wykorzystanych do budowy modelu danych utrudnia skuteczne określenie przynależności obiektów do poprawnych klas.
